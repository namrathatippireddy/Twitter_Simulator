defmodule Client do
  ...
  def handle_cast({:subscribe, num_users}, state) do
    ...
    Enum.each(1..numOfUsersToSubcribe, fn n->
      ...
      if(userToSub!=state["user_id"]) do
        #IO.inspect("subscribing in client")
  ...
  def handle_cast({:receiveTweet, user_id, tweet_content}, state) do
    ...
    #for simulation purpose not making retweets as it is taking forever
    # tested with unittests
    retweet_yes = Enum.random(0..0)
  ...
  def handle_call({:searchHashtags}, _from,state) do
    ...
    hashtags = GenServer.call(String.to_atom("engine"), {:search_hashtags, state["user_id"], hashtags},:infinity)
    {:reply, hashtags, state}
  end
  '''
  def handle_call({:search_hashtag_reply, tweets_for_hashtag}, state) do
  ...
  '''
  def handle_cast({:searchMentions, num_users}, state) do
    #IO.puts("Client will search for a mention")
    no_of_mentions =Enum.random(1..2)
    mentions =
      if(no_of_mentions ==1) do
      userid = Enum.random(1..num_users)
      mention = "@#{userid}"
      [mention]
    else
      userid1 = Enum.random(1..num_users)
      userid2 = Enum.random(1..num_users)
      mention1 = "@#{userid1}"
      mention2 = "@#{userid2}"
      [mention1, mention2]
    end
